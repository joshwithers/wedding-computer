  ...
  const candidates = await env.DB
    .prepare(
      `SELECT w.*,
         ...
         (SELECT COUNT(*) FROM run_sheet_items r WHERE r.wedding_id = w.id) AS rs_count,
         (SELECT MAX(r.updated_at) FROM run_sheet_items r WHERE r.wedding_id = w.id) AS rs_latest,
